...
the latent outcome variable (\code{yoX}) for the figure below, and
calculate the observable \code{xo}.  Note that the vectors of
explanatory variables for the selection (\code{xs}) and outcome
equation (\code{xo}) are independent and hence the exclusion
restriction is fulfilled.  This can also be seen from the fact that
the estimates are reasonably precise:
<<code:t2summary>>=
...
increasing in the single explanatory variable in the model, \code{xs}.
...
Monte-Carlo and real data examples.  Our implementation works well for
correctly specified cases, the problems appearing in the case of
misspecification or weak identification are related to the model
itself.  In these problematic cases a more robust maximisation
algorithm might be useful, even if the problems lie elsewhere.  More
robust optimisation is left for the future work.
