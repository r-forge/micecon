...
$\E [ \varepsilon_i^O|\varepsilon_i^S \ge -{\vec{\beta}^S}' \vec{x}_i^S ]
\not = 0$, unless $\varepsilon^O$ and $\varepsilon^S$ are uncorrelated
...
-{\vec{\beta}^S}' \vec{x}_i^S ]$, also called \emph{control function},
by estimating the selection equations \eqref{eq:probit*} and
\eqref{eq:probit} by probit, and thereafter insert these expectations
into equation \eqref{eq:outcome*} as additional covariates (see
\citealp{greene2002} for details):
\begin{equation}
  y_i^O
  =
  {\vec{\beta}^O}' \vec{x}_i^O 
...
covariance matrix of the coefficients of the probit estimation
...
(see the example in Secion~\ref{sec:convergence}).  The two-step
...
has a certain identifying power which we now have lost.  We are solely
...
@
%
Now all the parameters are precisely estimated, with even higher precision
...
Technically, there was a multicollinearity problem between $x^S$ and
$\lambda(\cdot)$.
...
\caption{The observable (filled) and unobservable (empty circles),
  ...
  the dotted line is the corresponding OLS estimate, based on
...
\begin{align}
  ...
  \end{cases}
\end{align}
We assume that the disturbance terms have a 3-dimensional normal
distribution:
\begin{equation}
  \begin{pmatrix}
    \varepsilon^S\\
...
exclusion restriction is fulfilled.  \code{selection} now expects
three formulas, one for the selection equation, as before, and a list
of two for the outcome equation.  Note that we do not have to remove
the unobserved values from outcome vectors, those are simply ignored.
...
(like in this case, where we use \code{set.seed(6)}), the results be
seriously biased.  Note that the first outcome parameters have low
