...
\Abstract{ 
  ...
  problem, the Heckman solution and argue that although the modern
  econometrics has superior non- and semiparametric estimation methods
  in it's toolbox, the Heckman models still serve as excellent
...
are twins, but neither can the reasearcher -- the twins may tend to be
born in a different types of families than singles.  All these cases
...
sample of interest, which is not any more ``random'' (see \citet[p
...
\item Random experiment, the situation, where the participants
  ...
  method is seldom feasible for pracitcal and ethical reasons.  The
...
all (this is why the example above uses \code{set.seed(5)}).
...
@
%
Now, in most cases the model does not converge.  However, if it does,
the results may be seriously biased:
<<>>=
summary(tmp <- selection(ys~xs, list(yo1 ~ xs, yo2 ~ xs), iterlim=50))
@ 
%
...
terms (Figure~\ref{fig:cfunctions}).  We use the estimated parameters
for constructing the latter, however, we scale the normal control
functions (inverse Mill's rations) to a rougly similar scale, as the
correct ones.
\begin{figure}[tb]
  \centering
<<fig=TRUE,height=4,echo=FALSE>>=
...
text(2.2, -1.4,
     expression(E*group("[", Epsilon^{O1}*group("|", 
         ...
         paste("  assumed normal")))
@ 
  \caption{Correct, and assumed normal, control functions.  Vertical
    lines denote the support of $x^S$ in the MC experiment.}
...
@ 
%
One can see, that OLS estimates are very close to the ML ones because
...
We illustrate this behaviour on a real individual data.  Consider the
following example:
<<init_2step>>=
data(Mroz87)
...
probability whie education and residence in a big city increase it.
...
variance).  The maximisation routine gets stuck in a local feature and
...
calculating the inital values, does a good job.
...
implementation in \pkg{micEcon} for programmin language \proglang{R}.
