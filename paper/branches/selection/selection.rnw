...
of the model in \pkg{micEcon}.  In Section~\ref{sec:using} we
...
used.  This results in a reasonably fast computation even in the case
...
print( try( selection( lfp ~ age + I( age^2 ) + faminc + kids + educ,
   wage ~ exper + I( exper^2 ) + educ + city, data = Mroz87,
   maxMethod = "BHHH" ) ) )
@
%
...
* I use an uft-8 locale and included \usepackage[utf8]{inputenc}.
  Remove it, if it doesn't work for you. 

* In general, one should use the names of coefficients, supplied by
  the user through initial values.  What if the initial values do not
  have any names?  I modified the code accordingly.
