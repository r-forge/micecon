...
to the values published in the errata of \citet{greene2002}
(\url{http://pages.stern.nyu.edu/~wgreene/Text/Errata/ERRATA5.htm}).
While we can obtain the same coefficients with the Newton-Raphson (NR)
maximization method,
we have to use the Bernd-Hall-Hall-Hausman (BHHH) method
to obtain the published standard errors.
This is because of different ways of calculating the Hessian
