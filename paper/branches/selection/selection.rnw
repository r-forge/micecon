...
Solving the sample selection problems requires additional
...
\item Random experiment, the situation, where the participants
  ...
  has.  Randomisation is often the best possible
...
art while estimating the causal effects.  However, methods relying 
...
fully parametric, it is straightforward to construct more efficient maximum likelihood (ML)
estimator.  Using the properties of
bivariate normal, it is easy to show that the log-likelihood can
be written as
\begin{align}
  \loglik 
  & = 
...
model may be relevant for treatment effect or migration analysis.  Other types of
...
algorithm (implemented as the function \code{maxNR} in \pkg{micEcon})
