...
\Abstract{ 
  ...
  integral part of modern econometrics textbooks and syllabus.  We
  describe the implementation of these models in package \pkg{micEcon}
  and illustrate the usage of the package on several Monte Carlo and
  real data examples.  Our examples demonstrate the effect of
  exclusion restrictions, identification at infinity and
  misspecification.  We demonstrate that the package can be used as an
...
is only partially implemented with \code{NA} in plac of the
...
support of $\vec{x}^S$ from $[0,1]$ to $[-5,5]$:
<<>>=
  xs <- runif(500, -5, 5)
  ys <- xs + eps[,1] > 0
yoX <- xs + eps[,2]
...
thereafter\footnote{One has to choose suitable \code{parscale}, for
  ...
  Section~\ref{sec:Greene22.8}}.  In by far the most cases, the 2-step
method, used for calculating the initial values, does a good job.
...
language \proglang{R}.  We argue that these models serve as useful
teaching tools because they are easy to implement and understand.
However, nowadays there exist superior estimation methods for censored
data.
...
5) Looking at the issue.  I have implemented switching the max methods.
