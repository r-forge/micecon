...
of the population --- is usually referred to as \emph{sample
...
During recent decades, either randomisation or the
pseudo-randomisation (natural experiments) has become the state-of-the
art while estimating the causal effects.  However, methods relying on instruments or
...
package \pkg{micEcon}, which is available on
...
of the model in \pkg{micEcon} and its possible generalisations.
...
this classification are available in \citealp{amemiya84,amemiya1985}).  This
...
the inverse Mill's ratio $\lambda(\cdot)$, whereas its exact form stems
...
functions \code{tobit2fit} and \code{tobit5fit} that do the actual ML estimation.
...
By default, the likelihood maximisation is performed with the Newton-Raphson
algorithm (implemented as \code{maxNR} in \pkg{micEcon})
using an analytic score vector and an analytic Hessian matrix.
This results in a reasonably fast computation even in case
