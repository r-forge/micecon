...
expectations $\E [ \varepsilon_i^O|\varepsilon_i^S \ge -{\vec{\beta}^S}'
\vec{x}_i^S ]$ by estimating the selection equations
\eqref{eq:probit*} and \eqref{eq:probit} by probit, and thereafter
insert these expectations into equation \eqref{eq:outcome*} as additional
...
\end{equation}
where
$\hat{\vec{\eta}}$ is the vector of residuals from the OLS estimation
of~\eqref{eq:tobit2_2step},
$n^O$ is the number of observations in this estimation, and
$\hat{\delta}_i = \hat{\lambda}_i ( \hat{\lambda}_i
+ \left. \hat{\vec{\beta}}^S \right. ' \vec{x}_i^S )$.
Finally, an estimator of the correlation between $\varepsilon^S$
and $\varepsilon^O$ can be obtained by
$\hat{\varrho} = \hat{\beta}^\lambda / \hat{\sigma}$.
...
below, while we also give a taste of the real data.
Recall example  22.8 from \citet[p.~786]{greene2002}
(section:~\ref{sec:Greene22.8}).
This model gives reasonable results,
but the results are sensitive to the start values.
Now we re-estimate the model specifying start values ``by hand''
