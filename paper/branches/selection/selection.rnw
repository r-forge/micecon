...
illustrate the usage of the package on various simulated and real
...
previous case.  However, now the slope is upward biased becuse $\E [
...
@
%
Now, we estimate the model by the two-step method.  For brevity, we
just report the estimated coefficients and not the full summaries here.
<<greene22.8TwoStep>>=
coef( selection( lfp ~ age + I( age^2 ) + faminc + kids + educ,
...
<<cameron16.6TwoStep>>=
coef( selection( selectEq, outcomeEq, data = RandHIE[ subsample, ],
...
Only a very few coefficients deviate at the seventh decimal place.
...
\code{"2step"}.  However, ML estimator may have problems at the boundary of the
...
We briefly described the implementation and usage of tobit-2 and
tobit-5 (switching regression) models in \pkg{micEcon}, and possible
generalisations of our \code{selection} function.  We
demonstrated the usage of the package using a number of simulated and
...
cases a more robust maximisation algorithm might be useful, or
two-step estimator might be preferrable.
