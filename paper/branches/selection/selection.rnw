...
twin.  Many of these cases are not under researcher's control.  
Often, the subjects can decide themselves, whether they take the
...
problem\footnote{Correcting for selectivity is necessary but not
  ...
  A common problem of different type is lack of common support between
...
\item Random experiment, the situation, where the participants
  ...
  has.  Randomisation is in many cases the best possible
...
\item Instruments (exclusion restrictions) are in many ways similar to
  ...
  Unfortunately, these two requirements tend contradict each other,
  and only seldom we have instruments of reasonable
  quality.
...
art while estimating the causal effects.  However, the methods, relying on the
distributional assumptions are are still widely used.  The reason
is obvious --- these methods are simple, widely available in software
packages, and they are part of common econometrics syllabus.
This is true even though parametric assumptions can only seldom be
justified, and their interference
may be hard to disentangle from the other sources of identification.

These models also serve as excellent teaching tools.
...
of the model in \pkg{micEcon} and it's possible generalisations.  
In Section~\ref{sec:using} we
illustrate the usage of the package on various simulated and real
data examples.  Section~\ref{sec:reliability} is devoted to
replication excercises where we compare
our results to the other examples in the literature;
Section~\ref{sec:problems} describes the robustness issues of the
method and our
...
This model and its derivations were introduced in 1970s and in 1980s.
...
used in addition to the Heckman model (see \citealp{powell1994} and
...
default using analytic score vector and analytic Hessian matrix are
...
maximisation algorithm can be chosen by argument
...
distinguised from continuous response, we
...
In some cases
two-step estimator is preferrable.
