...
to the results reported by \citet[p.~553]{cameron05}
--- even if they are compared with the seven-digit values
in the \proglang{Stata} output that is available on
...
seem to be wrong,
because they differ from the \proglang{Stata} output
...
@
%
Unfortunately, the maximisation of the likelihood function does not converge.
However, if we provide starting values for $\sigma$ and $\rho$
that are close to the (generally unknown) ML estimates,
the maximisation converges:
<<cameron16.6ML2>>=
summary( selection( selectEq, outcomeEq, data = RandHIE[ subsample, ],
   start = c( coef( tmp, part="full")[1:36], 1.6, 0.7 ) ), part="full" )
@
%
These results are nearly identical to the values reported
by \citet{cameron05}.
