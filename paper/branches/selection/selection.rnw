...
and are standard procedures in popular software packages
...
is not appropriate.
A consistent variance covariance matrix can be obtained by
\begin{equation}
\VAR \left[ \hat{\vec{\beta}}^O, \hat{\vec{\beta}}^\lambda \right]
= \hat{\sigma}^2
...
on its diagonal, and
$\VAR \left[ \hat{\vec{\beta}}^S \right]$ is the variance covariance matrix
of the coefficients of the probit estimation.
...
coef(tmp, part="full")[ c( "invMillsRatio", "rho", "sigma" ) ]
sqrt(diag(vcov(tmp, part="full")))[ "invMillsRatio" ]
@
...
Now, we estimate the model by the two-step method
<<cameron16.6TwoStep>>=
summary( tmp <- selection( selectEq, outcomeEq, data = RandHIE[ subsample, ],
...
of Table~16.1 seem to be wrong,
% You mean in Cameron (2005)? ....
because they differ from \citeauthor{cameron05}'s \proglang{Stata} output
