...
\Abstract{ 
  ...
  and illustrate the use of the package on several Monte Carlo and
  real data examples.  Our examples include the effect of exclusion
  restrictions, identification at infinity and specification problems,
  and demonstrate that the package can be used as an excellent
  teaching tool.
  %
}
...
package \pkg{micEcon}, which is available at CRAN
...
maximisation (implemented as \code{maxNR} in \pkg{micEcon}), where
analytic score vector and analytic Hessian matrix are used.  This
results in a reasonably fast computation even in the case of tens of
thousands observations.  A well-defined model should converge in less
than 15 iterations; in the case of weak identification this number may
be considerably larger.  Convergence issues may appear at the boundary
of the parameter space (e.g.\ if $\varrho \to 1$) or if the algorithm
does not find the global maximum (see Section~\ref{sec:convergence}).

The \code{selection} returns an object of class \code{selection}.  The
corresponding \code{summary}, \code{coef} and \code{vcov} methods
include an extra argument \code{part} which specifies which part of
the model to output.  One may choose either the full model (\code{part
  = "full"}, default), or the outcome equation only (\code{part =
  "outcome"}).
...
2$.  In that case $\E [\varepsilon^O|y^S = 1] \approx 0$ for a a
