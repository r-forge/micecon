aidsElaJacobian <- function( coef, share, price = NULL, formula = "AIDS" ) {
   ...
   createMatrix <- function( nGoods, nCoef, dim, symbol ) {
      result <- matrix( 0, nrow = nGoods^dim, ncol = nCoef )
      if( dim == 1 ) {
         rownames( result ) <- paste( symbol, c( 1:nGoods ) )
      } else if( dim == 2 ) {
         rownames( result ) <- paste( symbol, rep( 1:nGoods, each = nGoods ),
            rep( 1:nGoods, nGoods ) )
      }
   ...
   jacobian$exp      <- createMatrix( nGoods, nCoef, 1, "Ex" )
   jacobian$hicks    <- createMatrix( nGoods, nCoef, 2, "Eh" )
   jacobian$marshall <- createMatrix( nGoods, nCoef, 2, "Em" )
   ...
   ehName <- array( paste( "Eh", rep( 1:nGoods, nGoods ),
      rep( 1:nGoods, each = nGoods ) ), dim = c( nGoods, nGoods ) )
   emName <- array( paste( "Em", rep( 1:nGoods, nGoods ),
      rep( 1:nGoods, each = nGoods ) ), dim = c( nGoods, nGoods ) )
   ...
   if( formula == "AIDS" ) {
      ...
      for( i in 1:nGoods ) {
         ...
         jacobian$exp[ paste( "Ex", i ), bName[ i ] ] <-
            1 / share[ i ]
         for( j in 1:nGoods ) {
